const HotelRep = require ('../models/HotelRep');
const HotelSpace = require ('../models/HotelSpace');
...
// @access Private
const createHotelRep = asyncHandler(async (req, res) => {
    ...
    const duplicate =  await HotelRep.findOne({username : username}).lean().exec();
    if (duplicate) 
    ...
    const user = await HotelRep.create(accObject);
...
// @access Private
const getAllHotelReps = asyncHandler(async (req, res) => {
    const accounts = await HotelRep.find().select('-password').lean();
...
// @access Private
const getHotelRep = asyncHandler(async (req, res) => {
...
// @access Private
const updateHotelRep = asyncHandler(async (req, res) => {
    ...
    // check if exists
    const user = await HotelRep.findById(id).exec();
    if (!user)
        return res.status(400).json({ message : `Service account not found.` })

    // check for duplicate
    const duplicate = HotelRep.findOne({ username }).lean().exec() ;
...
// @access Private
const deleteHotelRep = asyncHandler(async (req, res) => {
    const { id } = req.body; 

    if (!id)
        return res.status(400).json({ message : `Account ID is required.` })
    /*
    const userspaces = await HotelSpace.findById({hotelRep_id : id}).lean().exec()
    if (userspaces?.length) {
        return res.status(400).json({ message : `Account has available spaces.` })
    }*/
    
    const account = await HotelRep.findById(id).exec() ;
